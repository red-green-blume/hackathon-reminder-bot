...
async def announce_winner(db_name, session_id, current_chat_id, bot):
    ...
    try:
        ...
        if not game_data:
            await bot.send_message(current_chat_id, "❌ Не удалось найти данные об игре.")
            return
        ...
        if last_word_user_id:
            ...
            winner_name = winner_name[0] if winner_name else f"Игрок {last_word_user_id}"
            ...
            await bot.send_message(
                current_chat_id,
                "К сожалению, победитель не определен."
            )
...
async def cmd_join(message: types.Message):
    ...
    if chat_id not in active_games:
        response = await message.answer("❌ В этом чате нет активной игры. Создайте игру командой /2_newgame")
    ...
    confirmation = await message.answer(f"✅ {message.from_user.full_name} присоединился к игре!")
...
@dp.message(Command("2_startgame"))
...
async def cmd_leave(message: types.Message):
    ...
    if session_status == 'started' and game.get('current_player') == user_id:
        ...
        if next_player_id:
            next_player_name = get_player_name(DB_NAME, next_player_id)
            await message.answer(f"🎯 Игрок вышел из игры. Следующий ход: {next_player_name}")
...
async def handle_game_message(message: types.Message):
    ...
    if game.get('last_word'):
        ...
        if not word.startswith(last_letter):
            await message.answer(f"❌ Слово должно начинаться на букву '{last_letter.upper()}'!")
            return

    translation = check_word_exists(DB_NAME, word)
    if not translation:
        await message.answer("❌ Это слово не найдено в словаре! Попробуйте другое слово.")
        return
